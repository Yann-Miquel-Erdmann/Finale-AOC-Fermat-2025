...
void egalite(phrase_t* phrase, bool constant) {
    if (phrase->args[0]->valeur->type == FLOAT || phrase->args[1]->valeur->type == FLOAT) {
        set_bool(phrase->valeur, get_as_float(phrase->args[0]->valeur) == get_as_float(phrase->args[1]->valeur));
    } else {
        set_bool(phrase->valeur, get_as_int(phrase->args[0]->valeur) == get_as_int(phrase->args[1]->valeur));
    }
