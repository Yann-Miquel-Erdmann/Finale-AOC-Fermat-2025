...
for line in data:
    ...
    if platform.system() == "Darwin" : # pour que erwan puisse faire les tests
        command = f"""/usr/local/bin/valgrind --error-exitcode=1 --leak-check=full --suppressions=testing/minimal.supp build/interpreter testing/python_test.FC """
    else:
        command = f"""valgrind --error-exitcode=1 --leak-check=full build/interpreter testing/python_test.FC """
